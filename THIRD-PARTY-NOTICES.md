...
| `assets/test-print/FDM_TEST.gcode` — **in this repo** | the printer's built-in test print: **#3DBenchy**, the well-known calibration model by **Creative Tools** (<https://www.3dbenchy.com>), re-sliced for the profile this kit ships. The geometry is untouched — only slicer settings differ, so this is a **format shift** (model → G-code), which even the model's original CC BY-ND terms permit. The model was placed in the **public domain** by its current rights holder (NTI Group) in 2025; we credit it regardless. | Model: **public domain** (previously CC BY-ND 4.0), © Creative Tools. Slicing profile and the G-code file: ours, AGPL-3.0. |
