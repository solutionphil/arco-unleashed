...
| `mainsail-theme/variants/*/custom.css` — **in this repo** | one icon path (the `usb` glyph) embedded as a `data:` URI, so the AMS / USB-stick connection indicators show a USB plug instead of the print-nozzle icon Mainsail uses for filament sensors. No image file and no icon font are shipped — the mask is the single path. | **Apache-2.0**, © [Pictogrammers](https://pictogrammers.com) (Material Design Icons), source <https://github.com/Templarian/MaterialDesign-SVG>. Apache-2.0 is one-way compatible with this project's AGPL-3.0. Full licence text: [`licenses/Apache-2.0.txt`](licenses/Apache-2.0.txt). |
...
| `assets/test-print/` — **in this repo** | the printer's two built-in test prints, `FDM_TEST.gcode` and the four-colour `Chroma_Kit_TEST.gcode`. Both are **#3DBenchy**, the well-known calibration model by **Creative Tools** (<https://www.3dbenchy.com>), re-sliced for the profile this kit ships; the colour one keeps the band heights of the file it replaces. The geometry is untouched — only slicer settings differ, so this is a **format shift** (model → G-code), which even the model's original CC BY-ND terms permit. The model was placed in the **public domain** by its current rights holder (NTI Group) in 2025; we credit it regardless. | Model: **public domain** (previously CC BY-ND 4.0), © Creative Tools. Slicing profiles and the G-code files: ours, AGPL-3.0. |
